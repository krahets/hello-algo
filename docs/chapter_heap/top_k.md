...
!!! question
    ...
    给定一个长度为 $n$ 的无序数组 `nums` ，请返回数组中最大的第 $k$ 个元素。
...
![遍历寻找最大的第 k 个元素](top_k.assets/top_k_traversal.png)
...
如下图所示，我们可以先对数组 `nums` 进行排序，再返回最右边的第 $k$ 个元素，时间复杂度为 $O(n \log n)$ 。

显然，该方法“超额”完成任务了，因为我们只需找出最大的第 $k$ 个元素即可，而不需要排序其他元素。

![排序寻找最大的第 k 个元素](top_k.assets/top_k_sorting.png)
...
4. 遍历完成后，堆中保存的就是最大的第 $k$ 个元素。

=== "<1>"
    ![基于堆寻找最大的第 k 个元素](top_k.assets/top_k_heap_step1.png)
...
另外，该方法适用于动态数据流的使用场景。在不断加入数据时，我们可以持续维护堆内的元素，从而实现最大的第 $k$ 个元素的动态更新。
