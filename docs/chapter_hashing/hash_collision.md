...
!!! note "工业界方案"
    ...
    Golang 采用「链式地址」。在Go语言中每个桶都能存储8个键值对，当哈希表中存储数据过多，单个桶已经装满时就会使用溢出桶来存储溢出的数据，溢出桶的设计能够减少扩容的频率。这两种桶拥有相同的数据结构，并且在内存中是连续存储的。Go语言中桶的底层数据结构只定义了一个字段来存储键的哈希的高8位，通过比较不同键的哈希的高8位可以减少访问键值对次数以提高性能。
