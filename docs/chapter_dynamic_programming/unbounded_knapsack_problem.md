...
- **放入物品 $i$** ：与 0-1 背包不同，转移至 $\max([i-1, c-wgt[i-1]] + val[i-1], [i-1, c - 2 * wgt[i-1]] + 2 * val[i-1], ...)$ 。

从而状态转移方程变为：

$$
dp[i, c] = \max(dp[i-1, c], dp[i-1, c-wgt[i-1]] + val[i-1], dp[i - 1, c - 2 * wgt[i-1]] + 2 * val[i-1], ...) {，式1}
$$

由上式可以将 $c$ 替换成 $c-wgt[i-1]$ ，每项加上 $val[i-1]$ 得:

$$
dp[i, c-wgt[i-1]] + val[i-1] = \max(dp[i-1, c-wgt[i-1]] + val[i-1], dp[i-1, c - 2 * wgt[i-1]] + 2 * val[i-1], dp[i-1, c - 3 * wgt[i-1]] + 3 * val[i-1], ...) {，式2}
$$

我们可以发现式2就是式1的后半部分，整理上述两个式子，就可以推导出完全背包的状态转移方程为：
