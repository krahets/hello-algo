...
fn merge(nums: []i32, left: usize, mid: usize, right: usize) !void {
    ...
    var i = leftStart;
    var j = rightStart;
    // Объединить левый и правый подмассивы, перезаписав исходный массив nums
    var k = left;
    while (k <= right) : (k += 1) {
        // Если «левый подмассив уже полностью объединен», выбрать элемент правого подмассива и выполнить j++
        if (i > leftEnd) {
            nums[k] = tmp[j];
            j += 1;
        // Иначе, если «правый подмассив уже полностью объединен» или «элемент левого подмассива <= элементу правого подмассива», выбрать элемент левого подмассива и выполнить i++
        } else if  (j > rightEnd or tmp[i] <= tmp[j]) {
            nums[k] = tmp[i];
            i += 1;
        // Иначе, если «оба подмассива еще не полностью объединены» и «элемент левого подмассива > элемента правого подмассива», выбрать элемент правого подмассива и выполнить j++
        } else {
            nums[k] = tmp[j];
            j += 1;
        }
