...
void bucketSort(float nums[], int n) {
    int k = n / 2;                                 // Инициализировать k = n/2 бакетов
    int *sizes = malloc(k * sizeof(int));          // Записать размер каждого бакета
    float **buckets = malloc(k * sizeof(float *)); // Массив динамических массивов (бакеты)
    // Предварительно выделить достаточно места для каждого бакета
    for (int i = 0; i < k; ++i) {
    ...
    // 3. Объединить бакеты после сортировки
    int idx = 0;
    for (int i = 0; i < k; ++i) {
