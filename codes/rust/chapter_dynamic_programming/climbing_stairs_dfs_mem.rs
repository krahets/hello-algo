...
/* 记忆化搜索 */
fn dfs(i: usize, mem: &mut [Option<usize>]) -> usize {
    // 已知 dp[1] 和 dp[2]，返回之
    // 实际上，这一处理会导致 mem 的前三个元素永远是 None，可以考虑
    // 将 mem 的大小设为 n - 2 并通过偏移索引来访问以节省空间；然而，
    // 索引偏移本身是有时间开销的，一般我们认为应该用空间换时间，所以
    // 这种优化并不值得。此外，Option<usize> 因为对齐会占据两倍字宽，
    // 可以考虑用 usize::MAX 来代表无记录，但这会带来可读性的下降。
    if i == 1 || i == 2 {
        return i;
    }

    // 若存在记录 dp[i]，则直接返回之
    if let Some(count) = mem[i] {
        return count;
    }
...
/* 爬楼梯：记忆化搜索 */
pub fn climbing_stairs_dfs_mem(n: usize) -> usize {
