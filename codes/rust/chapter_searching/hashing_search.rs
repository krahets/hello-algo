...
/* 哈希查找（数组） */
pub fn hashing_search_array(map: &HashMap<i32, usize>, target: i32) -> Option<&usize> {
...
fn main() {
    ...
    let index = hashing_search_array(&map, target).unwrap();
    println!("目标元素 3 的索引 = {index}");

    /* 哈希查找（链表） */
    let head = ListNode::try_from_array(nums).unwrap();
    // 初始化哈希表
    let map1 = HashMap::from(head);
