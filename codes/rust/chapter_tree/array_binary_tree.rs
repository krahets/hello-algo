...
impl ArrayBinaryTree {
    ...
    /* 深度优先遍历 */
    fn dfs(&self, i: usize, order: &str, res: &mut Vec<i32>) {
        let Some(val) = self.val(i) else { return };

        // 此处匹配字符串属于不良实践，它也许来自对其他语言实现的模仿。除了只读数据段和
        // 栈空间的额外开销，这还产生了一个冗余的 fallback 分支。一个较好的实现方式是
        // 将 order 定义为枚举，一个更好的实现方式是直接实现 pre_order、in_order
        // 和 post_order 方法，因为这里的动态性毫无意义。
        match order {
            // 前序遍历
            "pre" => {
                res.push(val);
