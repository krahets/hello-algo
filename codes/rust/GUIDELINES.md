...
- **可见性**：**示例代码**全部位于 bin crate，原则上不会被其他地方使用，最大可见性本应为 `pub(crate)`，在 crate 顶层无需被可见性修饰符修饰；然而，「实现某种数据结构或算法」暗示着代码将作为某种公开接口供下游使用，为了表达这种意图，以及区分公开接口和内部实现细节，**示例代码**中原则上是公开接口的部分应使用 `pub` 修饰符，是内部实现细节的部分不使用可见性修饰符。可见性修饰符中不得出现范围参数，例如 `pub(super)`、`pub(in some::path)` 等。
