...
// Assume maximum number of nodes is 100
#define MAX_SIZE 100
...
int isVisited(Vertex **visited, int size, Vertex *vet) {
    // Traversal search node, using O(n) time
...
void graphBFS(GraphAdjList *graph, Vertex *startVet, Vertex **res, int *resSize, Vertex **visited, int *visitedSize) {
    ...
    while (!isEmpty(queue)) {
        Vertex *vet = dequeue(queue); // Dequeue the vertex at the head of the queue
        ...
        while (node != NULL) {
            ...
            if (!isVisited(visited, *visitedSize, node->vertex)) {
                ...
                visited[(*visitedSize)++] = node->vertex; // Mark the vertex as visited
            }
            node = node->next;
        }
    }
...
int main() {
    ...
    Vertex *res[MAX_SIZE];
    int resSize = 0;
    // Used to record vertices that have been visited
