...
// Assume maximum vertex count is 100
#define MAX_SIZE 100
...
void addVertex(GraphAdjMat *graph, int val) {
    if (graph->size == MAX_SIZE) {
        fprintf(stderr, "Graph's vertex count has reached the maximum\n");
        return;
    }
    // Add the nth vertex, and set the nth row and column to zero
    int n = graph->size;
